#include "Player.h"
#include <iostream>
#include <iCardGame.h>


BlackJack_Player::BlackJack_Player(int Money, std::string Name) {   //   : _Money(Money), _Name(Name)
    _Money=Money;
    _Name=Name;
    _Insurance=0;
    _isBlackJack= false;
    _Choice='c';
}


void BlackJack_Player:: setChoice(int n) {

    if (n==11) {
        if (!(_isBlackJack)) {
            std::cout << getName() << ", Do you want to make an insurance (c - continue or i - insurance) ? : ";
            std::cin >> _Choice;
        }
        else{
            std::cout << getName()
            << ", Do you want to make an insurance or want to get payment (c - continue, i - insurance, p - Payment) ? : ";
            std::cin >> _Choice;
        }
    }


    if (_Money!=0&&n==11){

        if (_isBlackJack){
                std::cout<<"Stand, Payment or Insurance? :   "; // + insurance
                std::cin>>_Choice;
                while (_Choice!='s'&&_Choice!='e'&&_Choice!='p'&&_Choice!='i'){
                    std::cout<<"! Enter the correct character !"<<std::endl;
                    std::cout<<"Stand or Payment ? :   ";
                    std::cin>>_Choice;
                }
        }
    }
    else if (_Money!=0&&n==3){
        std::cout<<"Stand, Hit or Double ? : ";
        std::cin>>_Choice;
        while (_Choice!='h'&&_Choice!='s'&&_Choice!='e'&&_Choice!='d'){
            std::cout<<"! Enter the correct character !"<<std::endl;
            std::cout<<"Stand, Hit or Double ? :   ";
            std::cin>>_Choice;
        }
    }
    else{
        std::cout<<"Stand or Hit ? :   ";
        std::cin>>_Choice;
        while (_Choice!='h'&&_Choice!='s'&&_Choice!='e'){
            std::cout<<"! Enter the correct character !"<<std::endl;
            std::cout<<"Stand or Hit ? :   ";
            std::cin>>_Choice;
        }
    }
}


char BlackJack_Player::getChoice() {
    return _Choice;
}


void BlackJack_Player::Make_Double() {
    std::cout<<"You made a double: another "<<_Bet<<"$"<<std::endl;
    _Money-=_Bet;
    _Bet+=_Bet;
}


void BlackJack_Player::addCard(Card card) {
    _Hand.setCard(card);
}


void BlackJack_Player::setInsurance() {
    _Insurance = _Bet/2;
    _Money-=_Insurance;
    std::cout<<"You set an insurance bet: "<<_Insurance<<"\n\n";
}

int BlackJack_Player::getInsurance() {
    return _Insurance;
}

void BlackJack_Player::ClearHistory() {
    _Insurance=0;
    _isBlackJack= false;
    _Choice='c';
    _Hand.ClearCards();
}